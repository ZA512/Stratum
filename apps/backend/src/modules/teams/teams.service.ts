...
export class TeamsService {
  ...
  async bootstrapForUser(userId: string): Promise<{ team: TeamDto; rootNodeId: string; boardId: string }> {
    // Vérifier si l'utilisateur a déjà une team active
    const existingMembership = await this.prisma.membership.findFirst({
      where: { userId, status: MembershipStatus.ACTIVE },
      include: { team: { include: { memberships: { where: { status: MembershipStatus.ACTIVE }, select: { id: true } } } } },
  ...
  where: { teamId: existingMembership.teamId, parentId: null },
      ...
      if (rootComplex?.board?.id) {
        return { team: teamDto, rootNodeId: rootComplex.id, boardId: rootComplex.board.id };
      }
    ...
    const result = await this.prisma.$transaction(async (tx) => {
      ...
      const team = await tx.team.create({ data: { id: teamId, name: 'Mon Espace', slug: null } });
      await tx.membership.create({ data: { teamId: team.id, userId, status: MembershipStatus.ACTIVE } });
      ...
      if (!board) {
        ...
        const behaviors = await tx.columnBehavior.findMany({ where: { teamId: team.id } });
        const have = new Set(behaviors.map(b => b.key));
        const defaults: { key: ColumnBehaviorKey; label: string; color: string | null }[] = [
          { key: ColumnBehaviorKey.BACKLOG, label: 'Backlog', color: '#6b7280' },
          { key: ColumnBehaviorKey.IN_PROGRESS, label: 'En cours', color: '#2563eb' },
          { key: ColumnBehaviorKey.BLOCKED, label: 'Bloque', color: '#f97316' },
          { key: ColumnBehaviorKey.DONE, label: 'Termine', color: '#16a34a' },
        ];
        for (const def of defaults) {
          if (!have.has(def.key)) {
            await tx.columnBehavior.create({ data: { teamId: team.id, key: def.key, label: def.label, color: def.color } });
          }
        }
        const createdBehaviors = await tx.columnBehavior.findMany({ where: { teamId: team.id } });
        const map = new Map(createdBehaviors.map(b => [b.key, b.id]));
        const cols: { key: ColumnBehaviorKey; name: string; position: number; wipLimit: number | null }[] = [
          { key: ColumnBehaviorKey.BACKLOG, name: 'Backlog', position: 0, wipLimit: null },
          { key: ColumnBehaviorKey.IN_PROGRESS, name: 'En cours', position: 1, wipLimit: 5 },
          { key: ColumnBehaviorKey.BLOCKED, name: 'Bloque', position: 2, wipLimit: null },
          { key: ColumnBehaviorKey.DONE, name: 'Termine', position: 3, wipLimit: null },
        ];
        for (const col of cols) {
          const behaviorId = map.get(col.key);
          if (!behaviorId) continue;
          await tx.column.create({ data: { boardId: board.id, name: col.name, position: col.position, wipLimit: col.wipLimit, behaviorId } });
        }
      }
  await tx.node.update({ where: { id: rootNode.id }, data: { statusMetadata: { boardId: board.id } } });
    ...
    return { team: teamDto, rootNodeId: result.rootNodeId, boardId: result.boardId };
  }
}
